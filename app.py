...
top_metrics_html = "".join([
    ...
    metric_card_html("Tiers", f"{filtered_targets['tier'].nunique():,}", ", ".join(sorted(filtered_targets['tier'].unique())), "⚗", "blue"),
    metric_card_html("Aliases", f"{alias_count:,}", "Search expansion", "⌬", "teal"),
    metric_card_html("Scans today", "0", "Ready to run", "▥", "orange"),
])
...
<div class='registry-card'>
  <div class='registry-content'>
    ...
    <div class='registry-copy'>The registry is the control layer. Each target carries aliases, gene symbols, and known asset names so the scanner can catch trials that use drug, target, or biology-specific naming.</div>
    <div class='registry-meta'>
      <span class='registry-chip'>Target aliases</span>
      <span class='registry-chip'>Gene symbols</span>
      <span class='registry-chip'>Asset expansion</span>
      <span class='registry-chip'>Basket-ready</span>
    </div>
  </div>
</div>
""", unsafe_allow_html=True)
